...
export default function ExperienceDetailPage({ params }: { params: { slug: string } }) {
    ...
    // --- LÓGICA ACTUALIZADA PARA "MÁS EXPERIENCIAS" ---
    // Filtramos todas las experiencias para excluir solo la actual.
    // Esto asegura que siempre se muestren las demás experiencias disponibles.
    const relatedExperiences = experiences.filter(
        (exp) => exp.slug !== experience.slug
    );
    // --- FIN DE LA LÓGICA ---
        ...
        <div className="relative min-h-screen pb-20 bg-white overflow-x-hidden"> {/* Fondo de pantalla blanco */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 xl:grid-cols-12 gap-8 pt-[80px]">
                ...
                <div className="xl:col-span-8 overflow-x-hidden">
                    ...
                    {/* --- INICIO DEL NUEVO BLOQUE: MÁS EXPERIENCIAS --- */}
                    {relatedExperiences.length > 0 && (
                        <div className="mt-8 pb-12">
                            <h2 className={`text-xl md:text-2xl font-bold ${primaryColorClass} mb-4`}>
                                Más experiencias
                            </h2>
                            {/* Reemplazamos el Swiper por un scroll horizontal de cards */}
                            <div className="flex space-x-4 overflow-x-auto pb-4 scrollbar-hide">
                                {relatedExperiences.map((relatedExp) => (
                                    // Se ajusta el ancho de la tarjeta para que sea más pequeño
                                    <div key={relatedExp.product_id} className="w-64 flex-shrink-0 flex items-stretch">
                                        <ExperienceCard
                                            name={relatedExp.name}
                                            location={`${relatedExp.zone}, ${relatedExp.state}`}
                                            image={relatedExp.image}
                                            slug={relatedExp.slug}
                                            price={relatedExp.price}
                                            duration={relatedExp.duration}
                                            duration_type={relatedExp.duration_type}
                                            category={relatedExp.category}
                                            includes_food={relatedExp.includes_food}
                                        />
                                    </div>
                                ))}
                            </div>
                    ...
                    {/* --- FIN DEL NUEVO BLOQUE: MÁS EXPERIENCIAS --- */}
                </div>
